"""
Settings for eox_lms project meant to be called on the edx-platform/*/envs/aws.py module
"""
...
def plugin_settings(settings):  # pylint: disable=function-redefined
    ...
    if settings.SERVICE_VARIANT == "lms":
        if settings.EOX_CORE_APPEND_LMS_MIDDLEWARE_CLASSES:
            settings.MIDDLEWARE += [
                'eox_lms.middleware.PathRedirectionMiddleware',
                'eox_lms.middleware.RedirectionsMiddleware',
                'eox_lms.middleware.TPAExceptionMiddleware'
            ]
    ...
    if sentry_sdk is not None and sentry_integration_dsn is not None:
        from eox_lms.integrations.sentry import ExceptionFilterSentry  # pylint: disable=import-outside-toplevel
        sentry_sdk.init(
